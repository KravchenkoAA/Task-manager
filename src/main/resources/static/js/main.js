...
function checkActiveUser(authUser) {
    $("#helloMessage").text("Здравствуйте, " + authUser.name);
    if (authUser.activationCode != null) {
        $("#activateBlock").attr("hidden", false);
        $("#activateText").append("Для использования функциональности сервиса нужно завершить регистрацию")
        $("#settings")
            .removeAttr("class")
...
function addTask() {
    $("#addTaskButton").click(function (e) {
        if ($("#taskTitle").val() == "")
            $("#titleSpan").text("Введите заглавие");
        else
            $("#titleSpan").text("");

        if ($("#taskText").val() == "")
            $("#textSpan").text("Введите описание");
        else
            $("#textSpan").text("");

        var dateValidOne = validateDate($("#dateFinish").val());
        if ($("#dateFinish").val() === "") {
            $("#dateFinishSpan").text("Введите дату и время завершения");
        } else if (!dateValidOne) {
            $("#dateFinishSpan").text("Машину времени пока еще не придумали)");
        } else {
            $("#dateFinishSpan").text("");
            dateValidOne = true;
        }
        ...
        if ($("#dateToNotify").val() === "") {
            $("#dateToNotifySpan").text("Введите дату и время уведомления");
        } else if (!dateValidTwo) {
            $("#dateToNotifySpan").text("Машину времени пока еще не придумали)");
        } else {
        ...
        if ($("#taskPriority").val() == "")
            $("#taskPrioritySpan").text("Выберите приоритет");
        else
            ...
            $.ajax({
                ...
                success: function (result, status, xhr) {
                    if (result.description == "OK") {
                        $("#taskMessageSpan").text("Новая задача успешно добавлена");
                        setTimeout(function () {
                            location.reload();
                        }, 1500);
                    }
                ...
                error: function (xhr, status, error) {
                    ...
                    $("#taskMessageSpan").text("Упс... Что-то пошло не так");
                    setTimeout(function () {
                        location.reload();
                    }, 1500);
                }
...
function addTasklist() {
    $("#addListButton").click(function (e) {
        if ($("#listName").val() == "")
            $("#listNameText").text("Введите название списка");
        else
            $("#listNameText").text("");
        ...
        if ($("#listName").val() != "") {
            ...
            $.ajax({
                ...
                success: function (result, status, xhr) {
                    if (result.description == "OK") {
                        $("#listMessageSpan").text("Новый список задач успешно добавлен");
                        setTimeout(function () {
                            location.reload();
                        }, 1500);
                    } else {
                        $("#listMessageSpan").text("Упс... Что-то пошло не так");
                        setTimeout(function () {
                            location.reload();
                        }, 1500);
                    }
