...
function getTaskDetails(taskId, authUser) {
    $.getJSON('/task/details?id=' + taskId, function (data) {
        ...
        if (task.complete) {
            $("#status").attr('class', 'alert alert-success').append("Выполнена");
        } else {
            $("#status").attr('class', 'alert alert-warning').append("В работе");
        }
        ...
        $.ajax({
            ...
            success: function (result, status, xhr) {
                if (result.description == "OK") {
                    $("#taskMessageSpan").text("Вы поделились задачей с выбранными пользователями");
                    $.ajax({
                        type: "POST",
    ...
    $('#changeTaskButton').click(function () {
            ...
            if ((dateValidOne) && (dateValidTwo)) {
                ...
                $.ajax({
                    ...
                    success: function (result, status, xhr) {
                        if (result.description == "OK") {
                            $("#changeTaskMessageSpan").text("Задача успешно изменена");
                            setTimeout(function () {
                                location.reload();
                            }, 2000);
                        }
                    ...
                    error: function () {
                        $("#changeTaskMessageSpan").text("Упс... Что-то пошло не так...");
                    }
                });
...
function addComment(taskId, authUser) {
    $('#addCommentButton').click(function () {
            if ($("#inputComment").val() == "") {
                $("#commentHelp").text("Напишите что-нибудь");
            } else {
                var comment = {
