    ...
    $.getJSON('/check', function (data) {
        if (data.name != null) {
            ...
            $("#needUl").prepend('<li class="nav-item active">\n' +
                '<a class="btn btn-success" href="main.html" vi>Личный кабинет</a>\n' +
                '</li>');
            $("#message").text("Выйти");
            $("#url").attr("href", "/logout");
            $("#helloMessage").text("Здравствуйте, " + data.name);
        } else {
            $("#figure").attr('hidden', false);
            $("#message").text("Войти");
            $("#url").attr("href", "/login");
            $("#needUl").append('<li class="nav-item active">\n' +
                '<a class="btn btn-success" href="registration.html">Зарегистрироваться</a>\n' +
                '</li>');
            $("#helloMessage").text("Добро пожаловать, Гость");
        }
