...
public class SMSCSender implements SmsSender {
    ...
    private int translit = 0; //1 - переводить в translit
    private String time = "";
    private String id = "";
    ...
    public void sendSms(TaskToNotifyDTO task) {
        ...
        String[] result = {};
        String textMessage = "Uvazhaemyj " + transcriptor.getTranslit(task.getName()) + "!" + '\n' +
        ...
        if (SMSC_DEBUG) {
            if (Integer.parseInt(result[1]) > 0) {
                System.out.println("Сообщение отправлено успешно. ID: " + result[0] +
                        ", всего SMS: " + result[1] +
                        ", стоимость: " + result[2] +
                        ", баланс: " + result[3]);
            } else {
                System.out.print("Ошибка №" + Math.abs(Integer.parseInt(result[1])));
    ...
    public String[] getSmsCost(String phones, String message, int translit, int format, String sender, String query) {
        ...
        if (SMSC_DEBUG) {
            if (Integer.parseInt(result[1]) > 0) {
                System.out.println("Стоимость рассылки: " + result[0] + ", Всего SMS: " + result[1]);
            } else {
                System.out.print("Ошибка №" + Math.abs(Integer.parseInt(result[1])));
            }
        }
        ...
        try {
            ...
            if (SMSC_DEBUG) {
                if (result[1] != "" && Integer.parseInt(result[1]) >= 0) {
                    ...
                    System.out.println("Ошибка №" + Math.abs(Integer.parseInt(result[1])));
            }
    ...
    public String getBalance() {
        ...
        if (SMSC_DEBUG) {
            if (result.length == 1)
                System.out.println("Сумма на счете: " + result[0]);
            else
                System.out.println("Ошибка №" + Math.abs(Integer.parseInt(result[1])));
        }
